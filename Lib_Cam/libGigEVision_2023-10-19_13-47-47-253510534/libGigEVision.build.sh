...
DEFINES="${DEFINES} -D$($BIN_ECHO ${PROJECT_NAME^^} | $BIN_CUT -c4-)_PLATFORM_${TARGET_PLATFORM^^} -D$($BIN_ECHO ${PROJECT_NAME^^} | $BIN_CUT -c4-)_ARCHITECTURE_${TARGET_ARCHITECTURE^^}"
...
$BIN_ECHO "$(Now) [ConfigurationData] "'$BIN_ARCHIVER'"                 : $BIN_ARCHIVER"
...
FILES_SOURCES=$($BIN_FIND ${DIRECTORY_SOURCES} -type f -name "*.c")
...
case "$TARGET_PLATFORM" in
    Windows)
        TARGET_NAME_LIBRARY_DYNAMIC="${PROJECT_NAME}.dll"
        ;;
    *)
        TARGET_NAME_LIBRARY_DYNAMIC="${PROJECT_NAME}.so"
        ;;
esac

################################################################################

COMMAND_LINK="$BIN_COMPILER -shared ${CFLAGS} ${DEFINES} ${INCLUDES} -o ${DIRECTORY_TARGET}/${TARGET_NAME_LIBRARY_DYNAMIC} ${FILES_OBJECTS}"
...
case "$BUILD_TYPE" in
    Release)
        ...
        COMMAND_STRIP_SHARED_LIBRARY="$BIN_STRIPPER -s ${DIRECTORY_TARGET}/${TARGET_NAME_LIBRARY_DYNAMIC}"

        if [ "$BUILD_PRINT_COMMANDS" == true ]; then
            PrintLine
            $BIN_ECHO "$(Now) [Build] [StripSharedLibrary] $COMMAND_STRIP_SHARED_LIBRARY"
        fi

        $COMMAND_STRIP_SHARED_LIBRARY
...
case "$BUILD_TYPE" in
    Release)
        ...
        COMMAND_STRIP_OBJECTS="$BIN_STRIPPER -s $FILES_OBJECTS"

        if [ "$BUILD_PRINT_COMMANDS" == true ]; then
            PrintLine
            $BIN_ECHO "$(Now) [Build] [StripObjects] $COMMAND_STRIP_OBJECTS"
        fi

        $COMMAND_STRIP_OBJECTS
...
case "$TARGET_PLATFORM" in
    Windows)
        TARGET_NAME_LIBRARY_STATIC="${PROJECT_NAME}.lib"
        ;;
    *)
        TARGET_NAME_LIBRARY_STATIC="${PROJECT_NAME}.a"
        ;;
esac

################################################################################

COMMAND_ARCHIVE="$BIN_ARCHIVER -crs ${DIRECTORY_TARGET}/${TARGET_NAME_LIBRARY_STATIC} ${FILES_OBJECTS}"

if [ "$BUILD_PRINT_COMMANDS" == true ]; then
    PrintLine
    $BIN_ECHO "$(Now) [Build] [ArchiveObjects] $COMMAND_ARCHIVE"
fi
